...
class SpotROS():
    ...
    def RearImageCB(self, results):
        ...
        if data:
            ...
            image_msg1, camera_info_msg1 = getImageMsg(data[1], self.spot_wrapper)
            self.back_depth_pub.publish(image_msg1)
    ...
    def handle_dock(self, req):
        """ROS service handler for docking"""
        resp = self.spot_wrapper.dock()
        return TriggerResponse(resp[0], resp[1])

    def handle_undock(self, req):
        """ROS service handler for undockcing"""
    ...
    def main(self):
        ...
        if self.spot_wrapper.is_valid:
            ...
            self.feet_pub = rospy.Publisher('status/feet', FootStateArray, queue_size=100)
            ...
            while not rospy.is_shutdown():
                ...
                if self.has_cam:
                    self.cam_state.header.stamp = rospy.Time.now()
                    self.cam_pub.publish(self.cam_state)
                rate.sleep()
