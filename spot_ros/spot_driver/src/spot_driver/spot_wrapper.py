...
class SpotWrapper():
    ...
    def __init__(self, username, password, hostname, logger, estop_timeout=9.0, dock_id=520, has_arm=False, has_cam=False, rates = {}, callbacks = {}):
        ...
        if self._robot:
	        ...
	        # [TODO] Data receiving speed acceleration code
            self._async_tasks = AsyncTasks([self._robot_state_task])
            
            """
            self._async_tasks = AsyncTasks(
                [self._robot_state_task, self._robot_metrics_task, self._lease_task, self._idle_task])
    ...
    def dock(self):
        ...
        try:
            with LeaseKeepAlive(self._lease_client):
                ...
                blocking_dock_robot(self._robot, self._dock_id)
                return True, "Success"
        except Exception as e:
            return False, str(e)
